...
#        handle  -lpthread -lm -lcrypt -ldl -laio -lnuma  and other strings inserted that might contain
...
my $socket  = '/tmp/mysql_3306.sock';
my $version = '5.6.28-ndb-7.4.10';
...
my $ldata   = '/nvme_ssd/jsm1/mysql-cluster_2/data';
my $execdir = '/usr/local/kcj_cluster/mysql/bin';
my $bindir  = '/usr/local/kcj_cluster/mysql/bin';

# ----------------------------------------------------------------------
# If installed, search for the compiled in directory first (might be "lib64")
# ----------------------------------------------------------------------

my $pkglibdir = fix_path('/usr/local/kcj_cluster/mysql/lib',"libmysql/relwithdebinfo",
                         "libmysql/release","libmysql/debug","lib/mysql","lib");

my $pkgincludedir = fix_path('/usr/local/kcj_cluster/mysql/include', "include/mysql", "include");
...
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'',' -lpthread -lm -lcrypt -ldl -laio -lnuma ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','',' -lpthread -lm -lcrypt -ldl -laio -lnuma ','',''];

$flags->{include} = ["-I$pkgincludedir"];
$flags->{cflags}  = [@{$flags->{include}},split(" ",'  -O3 -g -fabi-version=2 -fno-omit-frame-pointer -fno-strict-aliasing -DDBUG_OFF')];
$flags->{cxxflags}= [@{$flags->{include}},split(" ",'-std=gnu++11  -O3 -g -fabi-version=2 -fno-omit-frame-pointer -fno-strict-aliasing -DDBUG_OFF')];
