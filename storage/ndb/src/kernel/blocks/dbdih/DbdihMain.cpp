...
//KCJ
//static Uint32 currentNodeID_KCJ = 10;
static Uint32 isNode_participating_num = 2; 
//static Uint32 isNode_participating_num_sub1 = 3;
//static Uint32 isNode_participating_num_sub2 = 4;
//static Uint32 isNode_participating_num_sub3 = 5;
...
#define sendLoopMacro_KCJ(sigName, signalRoutine, extra, selectedNode1, selectedNode2) \
{                                                                                   \
	  c_##sigName##_Counter.clearWaitingFor();                                          \
	  NodeRecordPtr specNodePtr;                                                        \
	  specNodePtr.i = cfirstAliveNode;                                                  \
	  do {                                                                              \
		      jam();                                                                          \
		      ptrCheckGuard(specNodePtr, MAX_NDB_NODES, nodeRecord);                          \
		      if (specNodePtr.i == selectedNode1 || specNodePtr.i == selectedNode2) {         \
			              c_##sigName##_Counter.setWaitingFor(specNodePtr.i);                         \
			              signalRoutine(signal, specNodePtr.i, extra);                                \
			          }                                                                               \
		      specNodePtr.i = specNodePtr.p->nextNode;                                        \
...
{ 
  printf("KCJ: Dbdih::sendCOPY_GCIREQ\n");
...
void Dbdih::execSTART_LCP_REQ(Signal* signal)
{
 ...
 // Uint32 nodeid_kcj = currentNodeID_KCJ;
 // BlockReference ref = calcDihBlockRef(nodeid_kcj);
  
 // printf("KCJ: Dbdih::execSTART_LCP_REQ > nodeid_kcj: %llu, ref: %llu\n", nodeid_kcj, ref);
      ...
      NDBD_SUPPORT_PAUSE_LCP)
  {
    if (req->pauseStart == StartLcpReq::PauseLcpStartFirst)
    {
      ...
      //KCJ
  //    printf("KCJ: execSTART_LCP_REQ: Modify1\n");
 //     ndbrequire(!req->participatingDIH.get(nodeCounter_KCJ));
 //     c_lcpState.m_participatingDIH.set(nodeCounter_KCJ);
...
/*
 ...
 req->participatingLQH.clear();
 req->participatingDIH.setEach(10,isNode_participating_num);
 req->participatingLQH.setEach(10,isNode_participating_num);
 c_lcpState.m_participatingDIH = req->participatingDIH;
 c_lcpState.m_participatingLQH = req->participatingLQH;

 if (isNode_participating_num == 21) {
	 isNode_participating_num = 11;
 } else {
	 isNode_participating_num++;
 }
...
void Dbdih::initLcpLab(Signal* signal, Uint32 senderRef, Uint32 tableId) 
{
  ...
  printf("KCJ: Dbdih::initLcpLab: senderRef: %llu\n", senderRef);
...
void Dbdih::packTableIntoPagesLab(Signal* signal, Uint32 tableId) 
{
  ...
  printf("CJ: Dbdih::packTableIntoPagesLab() \n");
...
void Dbdih::packFragIntoPagesLab(Signal* signal, RWFragment* wf) 
{
  ...
  wf->fragId++;
  printf("KCJ: Dbdih::packFragIntoPagesLab: front of switch ~ case \n");
  if (wf->fragId == wf->totalfragments) {
    ...
    printf("KCJ: Dbdih::packFragIntoPagesLab: %llu\n", wf->rwfTabPtr.p->tabCopyStatus);
...
Dbdih::startLcpMutex_locked(Signal* signal, Uint32 senderData, Uint32 retVal){
  ...
  */
  ...
  req->participatingDIH.clear();
  req->participatingLQH.clear();
  req->participatingDIH.setEach(1,isNode_participating_num);
  req->participatingLQH.setEach(1,isNode_participating_num);
  ...
  sendLoopMacro_KCJ(START_LCP_REQ, sendSTART_LCP_REQ, RNIL, 1, isNode_participating_num);

 // sendLoopMacro(START_LCP_REQ, sendSTART_LCP_REQ, RNIL);
   
  if (isNode_participating_num == 11) {
	         isNode_participating_num = 2;
  } else {
         isNode_participating_num++;
  }
...
Dbdih::sendSTART_LCP_REQ(Signal* signal, Uint32 nodeId, Uint32 extra)
{
  ...
  //nodeId = nodeCounter_KCJ;	
	
  BlockReference ref = calcDihBlockRef(nodeId);
  printf("KCJ: Dbdih::sendSTART_LCP_REQ: nodeId: %llu\n", nodeId); 
...
Dbdih::execSTART_LCP_CONF(Signal* signal)
{
 ...
 //  nodeId = nodeCounter_KCJ;
  ...
  mutex.unlock(c);
  printf("KCJ: Dbdih::execSTART_LCP_CONF: nodeCounter_KCJ: %llu\n", nodeId);
}
...
Dbdih::startLcpMutex_unlocked(Signal* signal, Uint32 data, Uint32 retVal){
  ...
  /* ----------------------------------------------------------------------- */
//KCJ
//  c_lcpState.m_participatingLQH.set(nodeCounter_KCJ);
...
{
  printf("KCJ:Dbdih::master_lcp_fragmentMutex_locked\n");
  jamEntry();
  ndbrequire(retVal == 0);
...
{
  printf("KCJ: Dbdih::writeFragment\n");
